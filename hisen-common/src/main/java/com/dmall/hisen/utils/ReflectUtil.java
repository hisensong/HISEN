...
/**
 * File: RockMethodUtil.java
 * Description:Java 反射工具类
 * Author: hisensong
 * Datetime: 2016-05-19 11:49
 */
public class ReflectUtil {
    ...
    public static Object invokeMethod(
        ...
        if (args.length > 0) {
            ...
            return method.invoke(object, new Object[method.getParameterTypes().length]);
        }
    }
    ...
    public static Method getMatchingAccessibleMethod(
            ...
            Class[] parameterTypes) {
        // trace logging
        log.debug("Matching name=" + methodName + " on " + clazz);
        ...
        try {
            ...
            log.debug("Found straight match: " + method);
            log.debug("isPublic:" + Modifier.isPublic(method.getModifiers()));
        ...
        for (int i = 0, size = methods.length; i < size; i++) {
            if (methods[i].getName().equals(methodName)) {
                ...
                if (methodParamSize != paramSize) {
                ...
                } else if ((methodParamSize == paramSize)) {
                    boolean match = true;
                    if (methodParamSize == 0) {//均无参数的情况
                        break;
                    } else {
                        for (int n = 0; n < methodParamSize; n++) {
                            if (parameterTypes[n] != null && !isAssignmentCompatible(methodsParams[n], parameterTypes[n])) {

                                log.debug(methodsParams[n] + " is not assignable from "
                                        + parameterTypes[n]);
                    ...
                    if (match) {
                        ...
                        if (method != null) {
                            ...
                            log.debug(method + " accessible version of "  + methods[i]);
                            ...
                            try {
                                ...
                                log.error("Cannot setAccessible on method. Therefore cannot use jvm access bug workaround.", se);
                            }
                            cache.put(md, method);
                            return method;
                        }

                        log.info("Couldn't find accessible method.");
                    }
                }
            }
        }
